...
本评测的两个任务均采用 **Precision（精确率）**、**Recall（召回率）** 和 **F1-Score（F1 值）** 作为评价指标。

#### 1. Precision（精确率）

精确率用于衡量模型预测为正例的结果中，有多少比例是真正的正例。

公式：Precision = TP / (TP + FP)


- **TP（True Positives）**：预测为正，实际也为正的数量  
- **FP（False Positives）**：预测为正，但实际为负的数量  
- **说明**：精确率越高，说明模型的预测误报越少。

---

#### 2. Recall（召回率）

召回率用于衡量模型能够捕获到的实际正例的比例。

公式：Recall = TP / (TP + FN)

- **FN（False Negatives）**：预测为负，但实际为正的数量  
- **说明**：召回率越高，说明模型漏掉的正例越少。

---

#### 3. F1-Score（F1 值）

F1 值是精确率和召回率的调和平均数，用于综合衡量模型的准确性和覆盖率。

公式：F1 = 2 * (Precision * Recall) / (Precision + Recall)

- **说明**：  
  - F1 值越高，说明模型在精确率和召回率之间取得了较好的平衡。  
  - 当精确率和召回率差异较大时，F1 值能够反映模型性能的综合情况。

## 4 评测赛程

- **2026年2月**：评测任务发布 & 宣传  
- **2026年4月3日**：训练集、测试A榜（验证集）数据发布  
