...
### 2.1 命名实体识别任务
─作物(Crop, CROP)：杂粮作物的物种或类别名称，用于指明研究对象，例：谷子；
─品种(Variety/Cultivar, VAR)：审定/登记/推广的具体品种名称或代号，例：晋谷21；
─性状(Trait, TRT)：表型/农艺/品质/抗性等性状名称，例：株高、千粒重；
─生育时期(Growth Stage, GST)：作物生长发育阶段节点，例：出苗期、拔节期；
─基因(Gene, GENE)：与性状相关的功能基因/候选基因名称，例：Waxy；
─数量性状位点(QTL, QTL)：与数量性状相关的遗传位点/区间标识，例：qPH3.1；
─分子标记(Molecular Marker, MRK)：用于定位/选择的SSR/SNP/Indel/KASP等标记，例：SSR-Xgwm20、SNP_rs12345、KASP_Chr2_15Mb；
─染色体(Chromosome, CHR)：染色体编号或名称，例：Chr1；
─育种方法(Breeding Method, BM)：育种或选择的技术路线/方法，例：杂交育种；
─亲本/杂交组合(Parent/Cross, CROSS)：父本/母本材料及其杂交组合表达，例：A×B；
─非生物胁迫(Abiotic Stress, ABS)：由非生命因素引起、会影响生长发育与产量/品质的胁迫或试验处理条件；例：干旱胁迫、高温胁迫；
─生物胁迫(Biotic Stress, BIS)：由生物因素（害虫、病原物、杂草等）引起、会造成取食、侵染或竞争危害的胁迫类型名称；例：螟虫、蚜虫等。
